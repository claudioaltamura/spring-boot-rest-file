...
public class FileController implements FileApi {
  ...
  public ResponseEntity<List<FileMetaInfo>> listFiles() {
    final var files =
        storageService.loadAll().map(this::buildFileMetaInfo).collect(Collectors.toList());
  ...
  public ResponseEntity<FileMetaInfo> uploadFile(MultipartFile attachment) {
    ...
    URI location =
        ServletUriComponentsBuilder.fromCurrentRequest()
            ...
            .buildAndExpand(fileMetaInfo.getFileName())
            .toUri();
  ...
  public ResponseEntity<List<FileMetaInfo>> uploadFiles(List<MultipartFile> attachments) {
    final var fileMetaInfoList =
        attachments.stream().map(this::storeFile).collect(Collectors.toList());
